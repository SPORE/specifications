...
An optional list of parameters which could be in B<required_params> or B<optional_params>.
This list will be used to create a payload data structure.

The option C<payload> B<MUST NOT> used the options C<required_payload> or C<optional_payload>
which handle opaque payload.
...
=head3 optional_payload

An optional boolean which handle opaque payload.

=head3 required_payload

An optional boolean which handle opaque payload.

=head3 unattended_params

An optional boolean which allows unattented parameters.
These parameters will be added to the query part of the request URL.
